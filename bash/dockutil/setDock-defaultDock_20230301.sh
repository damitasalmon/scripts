...
# Loop through Apps and check if App is installed, If Installed at App to the Dock. Addigy should check for this in conditions prior to execution. 
for app in "${apps[@]}"; 
do
	if [[ -e ${app} ]]; then
		${dockutil} --add "$app" --no-restart ${plist};
	else
		echo "${app} not installed. Install me please."
    fi
done
...
killall -KILL Dock
echo "Restarted the Dock for shiny newness"
